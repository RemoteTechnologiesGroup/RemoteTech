...
namespace RemoteTech
{
    ...
    {
        /// <summary>
        /// Get wether a vessel has local control or not (that is, if it's kerbal controlled or not).
        /// </summary>
        ...
        public static bool HasLocalControl(this Vessel vessel)
        {
            ...
            bool hasLocalControl = vessel.parts.Any(p => (p.isControlSource > Vessel.ControlLevel.NONE) && (p.protoModuleCrew.Any() || !p.FindModulesImplementing<ISignalProcessor>().Any()));
            if (!hasLocalControl)
            {
